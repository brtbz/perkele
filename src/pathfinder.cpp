...
void AnalyzeMapNodeFloodVersion(int32_t map_index, int32_t island_id)
{
	map_nodes[map_index].pathfind_island_id = island_id;
	int32_t neighbour;
	for (int i = 0; i < 6; i++)
	{
		if ( map_nodes[ map_index ].edge[i] != -1 )
		{
			...
			if (!MapIndexIsInClosedSet(neighbour) && !MapIndexIsInOpenSetMapIndices(neighbour))
			{
				...
				{
					AddOpenSetLeaf( 0.0f, 0, neighbour, 0 );
					AddOpenSetMapIndicesLeaf(neighbour);
				}
			}
		}
...
void FloodFillWith(int32_t map_index, int32_t island_id)
{
	AddOpenSetLeaf( 0.0f, 0, map_index, -1 );
	AddOpenSetMapIndicesLeaf(map_index);
	while (!OpenSetIsEmpty())
	{
		int32_t map_index;
		int32_t dont_care;
		int32_t dont_care2;
		PullMapIndexWithLowestFScoreFromOpenSet(&map_index, &dont_care, &dont_care2);
		AnalyzeMapNodeFloodVersion(map_index, island_id);
	}
}
...
void FloodFillIslandIds()
{
	...
	{
		if ( map_nodes[i].terrain == PASSABLE && map_nodes[i].pathfind_island_id < 0 )
		{
			FloodFillWith(i, island_id);
			island_id++;
		}
	}
}
