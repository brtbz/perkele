...
void main()
{
	...
	x_offset = 32.0f * float( terrain_type % 8 ) + 0.0f + margin;
	y_offset = 48.0f * float( terrain_type / 8 ) + 0.0f + margin;

	vec4 sprite_rectangle = vec4( x_offset, y_offset, 32.0f - 0.0f * margin, 48.0f - 0.0f * margin);

	//float highlight_x_offset = 64.0f * 1.0f + 1.0f + margin;
	//float highlight_y_offset = 53.0f * 3.0f + 1.0f + margin;

	int highlight_id = 73;
	float highlight_x_offset = 32.0f * float( highlight_id % 8 ) + 0.0f + margin;
	float highlight_y_offset = 48.0f * float( highlight_id / 8 ) + 0.0f + margin;

	if ( hex_index == highlighted_hex )
	{
		//highlight_x_offset = 64.0f * 0.0f + 1.0f + margin;
		//highlight_y_offset = 53.0f * 3.0f + 1.0f + margin;
		highlight_id = 90; //33;
		highlight_x_offset = 32.0f * float( highlight_id % 8 ) + 0.0f + margin;
		highlight_y_offset = 48.0f * float( highlight_id / 8 ) + 0.0f + margin;
	}

	vec4 highlight_sprite_rectangle = vec4( highlight_x_offset, highlight_y_offset, 32.0f - 0.0f * margin, 48.0f - 0.0f * margin);
	...
	highlight_frag_uv_coord.x = ( uv_coord.x / (sprite_sheet_size.x / highlight_sprite_rectangle.z) ) + ( highlight_sprite_rectangle.x / sprite_sheet_size.x );
	highlight_frag_uv_coord.y = ( uv_coord.y / (sprite_sheet_size.y / highlight_sprite_rectangle.w) ) + ( highlight_sprite_rectangle.y / sprite_sheet_size.y );
}
